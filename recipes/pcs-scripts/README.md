...
lifecycle — the `nodeBootstrapped` stage, for instance — which is where you'd mount
storage, tune the OS, join a directory, or tag the instance.
...
The full configuration reference — `onError`, `executionPolicy`,
`scriptCachingPolicy`, checksums, the injected context variables, and logging — is in
