...
SLURM_NODE_NAME=$($SCONTROL_PATH show node | grep -B16 "InstanceId=$INSTANCE_ID" | grep NodeName | awk -F= '{print $2}' | awk '{print $1}')
PARTITION_NAME=$($SCONTROL_PATH show node | grep -B16 "InstanceId=$INSTANCE_ID" | grep Partitions | awk -F= '{print $2}' | awk '{print $1}')

sed -i 's/^              - target_label: instance$/            - target_label: instance/' otel_config/config-compute-template.yaml
sed -i 's/^                replacement:/              replacement:/' otel_config/config-compute-template.yaml
sed -i "/target_label: instance\b/,/replacement:/ s/replacement:.*/&\n            - target_label: slurm_node\n              replacement: '$SLURM_NODE_NAME'/" otel_config/config-compute-template.yaml
sed -i "/target_label: instance\b/,/replacement:/ s/replacement:.*/&\n            - target_label: instance_id\n              replacement: '$INSTANCE_ID'/" otel_config/config-compute-template.yaml
sed -i "/target_label: instance\b/,/replacement:/ s/replacement:.*/&\n            - target_label: instance_type\n              replacement: '$INSTANCE_TYPE'/" otel_config/config-compute-template.yaml
sed -i "/target_label: instance\b/,/replacement:/ s/replacement:.*/&\n            - target_label: availability_zone_id\n              replacement: '$AZ_ID'/" otel_config/config-compute-template.yaml
sed -i "/target_label: instance\b/,/replacement:/ s/replacement:.*/&\n            - target_label: pcs_cluster_id\n              replacement: '$PCS_CLUSTERID_TAG'/" otel_config/config-compute-template.yaml
sed -i "/target_label: instance\b/,/replacement:/ s/replacement:.*/&\n            - target_label: pcs_compute_node_group_id\n              replacement: '$PCS_COMPUTENODEGROUPID_TAG'/" otel_config/config-compute-template.yaml
...
python3 -u install_observability.py --node-type compute --prometheus-remote-write-url $PROM_PARAM_VALUE