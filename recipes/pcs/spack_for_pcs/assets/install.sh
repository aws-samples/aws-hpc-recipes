...
# # Derived from Spack Configs for AWS ParallelCluster postinstall.sh                        #
##############################################################################################

print_help() {
    cat <<EOF
Usage: install.sh [-fg] [-v] [spec] [spec...]

Installs Spack on a PCS node group with recommended configurations.
...
initialization.  Output can be monitored in /var/log/spack-install.log
...
setup_variables() {
    ...
    # Detect if on AL2 
    if grep -q "Amazon Linux 2" /etc/os-release
    then
        export os=alinux2
    else
        export os=other
    ...
    if grep -q "ec2-user" /etc/shadow
    then
        default_user="ec2-user"
    elif grep -q "centos" /etc/shadow
    then
        default_user="centos"
    elif grep -q "rocky" /etc/shadow
    then
        default_user="rocky"
    elif grep -q "ubuntu" /etc/shadow
    then
        default_user="ubuntu"
    else
        default_user=""
    ...
    install_dir_option="${INSTALL_DIRECTORY:-"/shared"}"
    # Installs in --directory unless SPACK_ROOT is set
    install_path=${SPACK_ROOT:-"${install_dir_option}/spack"}
...
set_cluster_defaults() {
    ...
    [ -z "${SLURM_ROOT}" ] && SLURM_ROOT="${SLURM_INSTALL_DIRECTORY}"
...
if ${install_in_foreground}; then
    ...
    nohup bash ${tmpfile} &> /var/log/spack-install.log &
    disown -a
fi
